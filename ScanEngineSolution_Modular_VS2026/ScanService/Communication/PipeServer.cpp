...
// ROLE   : HELLO/WELCOME + SCAN/QUERY/CANCEL/TELEMETRY handlers.
...
{
    // Hai thread/session:
    // 1) thread_ doc request tu Client.
    // 2) eventThread_ gui progress/result da duoc worker dua vao eventQueue_.
    running_.store(true);
...
void ClientSession::Join()
{
    ...
    if (eventThread_.joinable() && eventThread_.get_id() != std::this_thread::get_id())
    {
        eventThread_.join();
    }
}
...
{
    // Ham nay co the duoc goi tu Worker/EngineCallback.
    // Chi enqueue de Worker tra ve DLL nhanh, KHONG WriteFile truc tiep tai day.
...
void ClientSession::Run()
{
    ...
    running_.store(false);
    eventCv_.notify_all();
    logger_.Info(L"Client session disconnected: " + std::to_wstring(sessionId_));
}
