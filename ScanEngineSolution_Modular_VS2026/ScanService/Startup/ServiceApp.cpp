...
// ROLE   : Khoi dong/dung Engine, worker pool, pipe server, throttle va telemetry.
...
// Thu tu khoi dong co chu y: Engine phai san sang truoc khi worker nhan Job.
bool ServiceApp::Start()
{
    ...
    // Throttle co thread rieng; worker chi doc State().
    throttle_.Start();
    const unsigned int hardware = std::thread::hardware_concurrency();
    const std::size_t workers = (std::max)(2u, (std::min)(hardware == 0 ? 4u : hardware, 8u));
    // Tao worker pool mot lan khi Service start (2..8 thread).
    jobs_.Start(workers);
