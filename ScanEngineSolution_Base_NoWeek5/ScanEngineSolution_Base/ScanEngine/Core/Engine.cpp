...
// ROLE   : Public Engine API; generic file analysis and streaming callback.
// ============================================================================
...
namespace
{
    ...
    bool TryReadJsonU64(const char* json, const char* key, std::uint64_t& value)
    {
        double number = 0.0;
        if (!TryReadJsonNumber(json, key, number) || number < 0.0) return false;
        value = static_cast<std::uint64_t>(number);
        return true;
    }

    bool TryReadJsonU32(const char* json, const char* key, std::uint32_t& value)
    {
        std::uint64_t number = 0;
        if (!TryReadJsonU64(json, key, number) || number > 0xFFFFFFFFull) return false;
    ...
    EngineStatus FinishWithError(
        ...
        const wchar_t* message)
    {
        const EngineEventType type = status == EngineStatus::Cancelled
        ...
        ReportEvent(callback, userContext, type, stage, 100, status, message);
        return status;
    }
...
{
    if (g_initialized.exchange(true)) return EngineStatus::AlreadyInitialized;

    EngineConfig config{};
    if (configJsonUtf8 != nullptr)
    {
        double entropy = config.entropyThreshold;
        std::uint64_t maxBytes = config.maxEntropyBytes;
        std::uint32_t interval = config.progressIntervalMs;
        TryReadJsonNumber(configJsonUtf8, "entropyThreshold", entropy);
        TryReadJsonU64(configJsonUtf8, "maxEntropyBytes", maxBytes);
        TryReadJsonU32(configJsonUtf8, "progressIntervalMs", interval);
        if (entropy > 0.0 && entropy <= 8.0) config.entropyThreshold = entropy;
        if (maxBytes != 0) config.maxEntropyBytes = maxBytes;
        config.progressIntervalMs = interval;
    }
    ...
    void* userContext)
{
    ...
    if (!ReportEvent(callback, userContext, EngineEventType::Progress,
        EngineScanStage::Starting, 0, EngineStatus::Success, L"Starting scan"))
    {
        return FinishWithError(callback, userContext, EngineStatus::Cancelled, L"Scan cancelled");
    }
    ...
    if (!ReportEvent(callback, userContext, EngineEventType::Progress,
        EngineScanStage::OpeningFile, 5, EngineStatus::Success, L"Opening file"))
    {
        return FinishWithError(callback, userContext, EngineStatus::Cancelled, L"Scan cancelled");
    }

    ScanEngineInternal::FileMetadata metadata{};
    status = ScanEngineInternal::ReadFileMetadata(normalizedPath, metadata, win32Error);
    if (status != EngineStatus::Success)
        return FinishWithError(callback, userContext, status, L"Cannot read file metadata");

    if (!ReportEvent(callback, userContext, EngineEventType::Progress,
        EngineScanStage::ReadingMetadata, 15, EngineStatus::Success, L"File metadata read"))
    {
        return FinishWithError(callback, userContext, EngineStatus::Cancelled, L"Scan cancelled");
    }
    ...
    if (!ReportEvent(callback, userContext, EngineEventType::Progress,
        EngineScanStage::ApplyingRules, 90, EngineStatus::Success, L"Applying detection rules"))
    {
        return FinishWithError(callback, userContext, EngineStatus::Cancelled, L"Scan cancelled");
    }
    ...
    // Demo rules from the original assignment.
    ...
    if (!ReportEvent(callback, userContext, EngineEventType::Progress,
        EngineScanStage::BuildingResult, 95, EngineStatus::Success, L"Building result"))
    {
        return FinishWithError(callback, userContext, EngineStatus::Cancelled, L"Scan cancelled");
    }
    ...
    if (!ReportEvent(callback, userContext, EngineEventType::Result,
        EngineScanStage::Completed, 100, EngineStatus::Success,
