...
// ROLE   : CLI client: connect, HELLO, send commands, receive streaming events.
// ============================================================================
...
namespace
{
    ...
    void Usage()
    {
        std::wcout
            << L"Usage:\n"
            << L"  client.exe scan <path> [--priority low|normal|high] [--timeout ms]\n"
            << L"  client.exe query <jobId>\n"
            << L"  client.exe cancel <jobId>\n"
            << L"  client.exe telemetry\n"
            << L"  client.exe stress <path> [--count 20] [--priority low|normal|high]\n";
    }
}
...
int wmain(int argc, wchar_t* argv[])
{
    ...
    if (_wcsicmp(command.c_str(), L"stress") == 0 && argc >= 3)
    {
        ...
        std::wcout << L"Stress completed. count=" << count
            << L", failures=" << failures.load() << L'\n';
