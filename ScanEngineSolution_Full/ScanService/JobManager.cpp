...
void JobManager::WorkerLoop()
{
    while (running_.load())
    {
        ...
        CacheKey key{normalizedPath, lastWriteTime, fileSize};
        ...
        if (status == EngineStatus::Success)
        {
            ...
            if (hasResult)
            {
                CacheKey finalKey{normalizedPath, result.lastWriteTime, result.fileSize};
                if (!normalizedPath.empty()) cache_.Put(finalKey, result);
            }
            Notify(job);
            telemetry_.RecordSucceeded(duration, false);
        }
        else if (status == EngineStatus::Cancelled || job->cancelRequested.load())
        {
            FinishCancelled(job);
        }
        else
